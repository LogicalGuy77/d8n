...
export function useNodeStatus(walletAddress) {
  ...
  const fetchNodeStatus = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/status?wallet=${walletAddress}`);
      ...
      if (response.ok) {
        ...
        if (previousNodeRef.current && !nodeId) {
          console.log(`[FRONTEND] Workflow completed! Previous node: ${previousNodeRef.current}`);
        ...
        console.log(`[FRONTEND] Status request failed with status: ${response.status}`);
